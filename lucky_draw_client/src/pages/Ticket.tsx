  ...
  if (isError || !data) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-background p-4 text-center">
        <h2 className="text-2xl font-bold text-destructive font-heading mb-2">Tiket Tidak Ditemukan</h2>
        <p className="text-muted-foreground mb-8">Data peserta tidak valid atau belum terdaftar.</p>
        <Button onClick={() => navigate("/register")} className="h-12 px-8 rounded-xl bg-primary text-primary-foreground text-base">
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-background relative overflow-hidden">
      ...
      <div className="w-full max-w-sm z-10">
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-foreground font-heading mb-2">Pendaftaran Berhasil!</h1>
          <p className="text-muted-foreground text-lg">Hai <span className="font-semibold text-foreground">{data.full_name}</span>,<br/>ini adalah tiket Doorprize Anda.</p>
        </div>
        ...
        <div className="relative bg-card rounded-2xl overflow-hidden shadow-2xl shadow-primary/20 border-2 border-primary ring-4 ring-primary/10 flex flex-col w-full">
          ...
          <div className="bg-foreground text-background p-8 pt-10 pb-10 text-center relative overflow-hidden flex flex-col items-center">
            ...
            <h2 className="font-semibold text-sm uppercase tracking-[0.2em] text-primary/80 z-10 relative">Doorprize Event</h2>
            <h3 className="font-bold text-2xl mt-2 font-heading text-primary z-10 relative drop-shadow-sm leading-tight">
              HUT Lampung Barat <br/>Ke-35
            </h3>
          </div>
          ...
          <div className="p-8 pt-10 pb-10 text-center bg-gradient-to-b from-card to-muted/50">
            <p className="text-sm font-bold text-primary/80 uppercase tracking-widest mb-3">Kode Tiket Anda</p>
            <div className="text-[2.75rem] leading-none font-black text-foreground tracking-tighter drop-shadow-sm font-sans mb-2">
              {data.coupon_code}
            </div>
            <p className="text-xs text-muted-foreground mt-4 uppercase tracking-widest font-semibold">Simpan kode ini baik-baik</p>
          </div>
        </div>

        <div className="mt-10 space-y-4">
          <Button variant="outline" className="w-full flex items-center justify-center gap-2 h-14 rounded-xl border-2 border-primary text-primary hover:bg-primary hover:text-primary-foreground transition-all font-semibold text-base" onClick={() => window.print()}>
            <DownloadSimple weight="bold" className="w-5 h-5" />
            Simpan / Cetak Tiket
          </Button>
          <Button className="w-full h-14 rounded-xl bg-muted text-muted-foreground hover:bg-muted/80 hover:text-foreground font-semibold text-base transition-colors" onClick={() => navigate("/register")}>
            Daftar Peserta Lain
          </Button>
        </div>
      </div>
